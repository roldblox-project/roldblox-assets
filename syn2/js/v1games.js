...
class GameRenderer {
    ...
    mapCategoryElements() {
        ...
        containers.forEach(cat => {
            if (cat.container) {
                const carouselContainer = cat.container.closest('.carousel-container');
   ...
   checkForSearchQuery() {
    ...
    if (keywords && keywords.trim() !== '') {
        ...
        const firstHeader = document.querySelector('.d-md-flex.align-items-center.w-100');
        if (firstHeader) {
            const trendingHeader = firstHeader.querySelector('.d-flex.align-items-center');
            if (trendingHeader) {
                const h1 = trendingHeader.querySelector('h1');
                const a = trendingHeader.querySelector('a');
                if (h1) h1.style.display = 'none';
                if (a) a.style.display = 'none';
            }
            
            const nextCarousel = firstHeader.nextElementSibling;
            if (nextCarousel && nextCarousel.classList.contains('carousel-container')) {
                nextCarousel.remove();
            }
        }
        
        document.querySelectorAll('.d-flex.align-items-center.mt-4').forEach(header => {
            if (!header.closest('#search-results-section')) {
                header.remove();
            }
        });
            ...
            results.forEach(result => {
                ...
                if (result.data && result.data.success && result.data.data && result.data.data.length > 0) {
                    ...
                    if (elements) {
                        ...
                        if (elements.carousel) elements.carousel.style.display = '';
                    }
                } else {
                    if (elements) {
    ...
    toggleCategorySections(show) {
        this.categoryElements.forEach((elements, type) => {
            ...
            if (elements.carousel) elements.carousel.style.display = show ? '' : 'none';
        });
